...
#include <osgEarth/Clouds>
#include <osgEarth/LogarithmicDepthBuffer>
#include <osgEarth/ReverseZBuffer>
...
namespace
{
    ...
    Clouds::Options weather()
    {
        Clouds::Options options;
        options.wind.set(0,0,0);
        options.coverage = 0.8f;
        options.quality = Clouds::LOW;
        return options;
    }
...
TEST_CASE("Capture cloud rays over a local city", "[cloudraycity][.gl]")
{
    ...
    auto options=weather(); options.coverage=0.55f; options.quality=Clouds::BALANCED;
    options.crepuscularRays=true; options.rayQuality=Clouds::HIGH; options.rayHaze=0.5f;
    ...
    osg::ref_ptr<Clouds> layer=new Clouds(options); sky->setClouds(layer);
...
TEST_CASE("Clouds reuse map wind without motion discontinuities", "[clouds][cloudsharedwind][wind][.gl]")
{
    ...
    options.rayQuality = Clouds::LOW; options.rayHaze = 0.5f;
    osg::ref_ptr<Clouds> clouds = new Clouds(options);
...
// Background rays and model/terrain depth must remain consistent when the same cloudy sky opts into reverse-Z.
TEST_CASE("Reverse-Z preserves atmospheric clouds and model visibility", "[reverse-z][reverse-z-render][clouds]")
{
    osg::ref_ptr<SkyNode2> sky = new SkyNode2;
    osg::ref_ptr<Clouds> clouds = new Clouds(weather());
    sky->setClouds(clouds);
    Scene scene(sky, 256, 128);
    scene.draw(); scene.draw();
    auto standard = scene.pixels();
    Util::ReverseZBuffer controller;
    REQUIRE(controller.install(scene.viewer.get()).isOK());
    scene.viewer->getCamera()->removeFinalDrawCallback(scene.readback);
    controller.getPresentationCamera(scene.viewer->getCamera())->setFinalDrawCallback(scene.readback);
    scene.draw(); scene.draw();
    REQUIRE(difference(standard, scene.pixels()) < 0.005);
    REQUIRE(controller.getReport(scene.viewer->getCamera(), scene.context->getState()->getContextID()).activation ==
        Util::ReverseZBuffer::ACTIVE);
...
TEST_CASE("Shared wind keeps cloud and shadow density aligned", "[clouds][cloudsharedwind][wind][.gl]")
{
    ...
    osg::ref_ptr<Clouds> layer = new Clouds(options); layer->setWindLayer(wind);
    sky->setClouds(layer);
...
TEST_CASE("Ground cloud shadows track the visible wind-advected cloud column", "[clouds][cloudwind][.gl]")
{
    ...
    osg::ref_ptr<Clouds> layer = new Clouds(options);
    sky->setClouds(layer);
    ...
    for (double elevation : {35.0,80.0})
    {
        ...
        osg::Vec3d eye(6378139.0,0,0);
        scene.viewer->getCamera()->setViewMatrixAsLookAt(eye,eye+sun->direction,osg::Vec3d(0,0,1));
        for (auto quality : {Clouds::LOW,Clouds::BALANCED,Clouds::HIGH})
...
TEST_CASE("Cloud controls sanitize and round trip", "[clouds]")
{
    ...
    Clouds::Options copy(sanitized.getConfig());
    ...
    REQUIRE(sky->getClouds());
    CHECK(sky->getClouds()->getOptions().resolution == 768u);
}
...
{
    osg::ref_ptr<SkyNode2> sky = new SkyNode2;
    auto options = weather(); options.quality = Clouds::BALANCED;
    options.coverage = 0.55f; options.seed = 2; options.shadowStrength = 0.0f;
    options.rayQuality = Clouds::HIGH; options.rayDistance = 30000.0f;
    osg::ref_ptr<Clouds> layer = new Clouds(options);
    sky->setClouds(layer);
    Scene scene(sky,1280,720);
...
TEST_CASE("Cloud ray haze clears with decreasing coverage", "[clouds][cloudrays][cloudraycoverage][.gl]")
{
    ...
    for (bool ground : {false,true})
    {
        ...
        for (auto quality : {Clouds::LOW,Clouds::HIGH})
        {
            ...
            for (float coverage : {0.6f,0.5f,0.4f,0.25f,0.2f,0.1f,0.02f,0.001f,0.0f})
            {
                ...
                if (!ground && quality == Clouds::HIGH && (coverage == 0.6f || coverage == 0.02f))
...
TEST_CASE("Cloud ray controls are optional and bounded", "[clouds][cloudrays]")
{
    ...
    options.rayDistance = -1.0f; options.rayQuality = static_cast<Clouds::Quality>(99);
    osg::ref_ptr<Clouds> layer = new Clouds(options);
    CHECK(layer->getOptions().rayStrength == 1.0f);
    CHECK(layer->getOptions().rayDistance == 1000.0f);
    CHECK(layer->getOptions().rayQuality == Clouds::BALANCED);
...
TEST_CASE("Cloud rays follow lighting wind and independent views", "[clouds][cloudrays][.gl]")
{
    ...
    options.rayQuality = Clouds::BALANCED;
    osg::ref_ptr<Clouds> layer = new Clouds(options);
    sky->setClouds(layer);
...
TEST_CASE("Cloud rays stop at foreground material distance", "[clouds][cloudrays][.gl]")
{
    ...
    auto options = weather(); options.coverage = 0.55f; options.shadowStrength = 0.0f;
    osg::ref_ptr<Clouds> layer = new Clouds(options);
    sky->setClouds(layer); scene.draw();
    auto clear = scene.pixels();
...
{
    osg::ref_ptr<Clouds> layer = new Clouds;
    ...
    layer->setOptions(options);
    Clouds::Options copy(layer->getOptions().getConfig());
...
TEST_CASE("Procedural clouds render from ground through orbit and disable cleanly", "[clouds][.gl]")
{
    ...
    osg::ref_ptr<Clouds> layer = new Clouds(weather());
    sky->setClouds(layer);
    REQUIRE(sky->getClouds() == layer.get());
    scene.draw();
    auto cloudy = scene.pixels();
    REQUIRE(sky->getStateSet()->getDefinePair("OE_CLOUDS"));
    ...
    layer->setOptions(options); scene.draw();
    CHECK(difference(clear,scene.pixels()) < 0.0001);
    sky->setClouds(nullptr); scene.draw();
    CHECK(difference(clear,scene.pixels()) < 0.0001);
    CHECK(glGetError() == GL_NO_ERROR);
}
...
TEST_CASE("Cloud transport preserves nearby material shading", "[clouds][.gl]")
{
    ...
    sky->setClouds(new Clouds(options));
    scene.draw();
    auto actual = scene.pixels();
    float maximum = 0.0f;
    unsigned count = 0;
...
{
    osg::ref_ptr<SkyNode2> sky = new SkyNode2;
    auto options = weather(); options.quality = Clouds::BALANCED; options.coverage = 0.65f;
    osg::ref_ptr<Clouds> layer = new Clouds(options);
    sky->setClouds(layer);
...
TEST_CASE("Cloud earth configuration renders with terrain", "[clouds][.gl]")
{
    ...
    auto sky = findTopMostNodeOfType<SkyNode2>(root.get());
    REQUIRE(sky != nullptr);
    REQUIRE(sky->getClouds() != nullptr);
    REQUIRE(MapNode::get(root.get()) != nullptr);
    Scene scene(sky,512,256);
    LogarithmicDepthBuffer depth; depth.install(scene.viewer->getCamera());
    auto options = sky->getClouds()->getOptions(); options.wind.set(0,0,0);
    sky->getClouds()->setOptions(options);
    scene.skyView(500.0);
    for (unsigned i=0; i<12; ++i) scene.draw();
    auto actual = scene.pixels();
    REQUIRE(scene.save("clouds-terrain.png"));
    options.enabled = false; sky->getClouds()->setOptions(options);
    scene.draw();
...
TEST_CASE("Cloud grazing rays converge across altitude and sun changes", "[clouds][cloudbands][.gl]")
{
    ...
    for (double sun : {3.0,35.0})
    {
        ...
        if (quality == Clouds::BALANCED) REQUIRE(scene.save(name+"-actual.png"));
        auto reference = options; reference.samples = 512;
        layer->setOptions(reference); scene.draw();
        auto expected = scene.pixels();
        if (quality == Clouds::BALANCED) REQUIRE(scene.save(name+"-reference.png"));
        ...
        INFO(name);
        CHECK(error < 0.001);
        CHECK(bands < (quality == Clouds::LOW ? 0.0004 : 0.00015));
    }
}
