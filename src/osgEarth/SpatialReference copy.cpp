...
namespace
{
    ...
    void geodeticToGeocentric(osg::Vec3d* points, unsigned count, const Ellipsoid& em)
    {
    ...
    void geocentricToGeodetic(osg::Vec3d* points, unsigned count, const Ellipsoid& em)
    {
        for (unsigned i = 0; i < count; ++i)
        {
    ...
    using TransformSRSID = std::uint64_t;

    struct TransformCacheKey
    {
        TransformSRSID source;
        TransformSRSID target;

        bool operator==(const TransformCacheKey& rhs) const
        {
    ...
    {
        std::size_t operator()(const TransformCacheKey& key) const
        {
            std::size_t result = std::hash<TransformSRSID>()(key.source);
            const std::size_t target = std::hash<TransformSRSID>()(key.target);
            result ^= target + 0x9e3779b9u + (result << 6) + (result >> 2);
            return result;
        }
    };

    // Intern each WKT once so transform-cache keys stay small and cache hits do
    // not allocate a temporary concatenated string.
    struct TransformCache
    {
        TransformCacheEntry& get(const std::string& sourceWKT, const std::string& targetWKT)
        {
            TransformCacheKey key = { getSRSID(sourceWKT), getSRSID(targetWKT) };
            return entries[key];
        }
    ...
    private:
        TransformSRSID getSRSID(const std::string& wkt)
        {
            auto i = srsIDs.find(wkt);
            if (i != srsIDs.end())
                return i->second;

            const TransformSRSID id = nextSRSID++;
            return srsIDs.emplace(wkt, id).first->second;
        }

        TransformSRSID nextSRSID = 0u;
        std::unordered_map<std::string, TransformSRSID> srsIDs;
        std::unordered_map<TransformCacheKey, TransformCacheEntry, TransformCacheKeyHash> entries;
    };
                            ...
                            osg::Vec3d&             output) const
{
    ...
    osg::Vec3d temp = input;

    if (transform(&temp, 1u, outputSRS))
    {
        output = temp;
        return true;
    }
...
bool
SpatialReference::transform(osg::Vec3d*              points,
                            unsigned                 count,
                            const SpatialReference*  outputSRS) const
{
    OE_SOFT_ASSERT_AND_RETURN(outputSRS != nullptr, false);
    OE_SOFT_ASSERT_AND_RETURN(points != nullptr || count == 0u, false);

    // Derived SRS classes can customize vector transforms with pre/post hooks.
    // Preserve those hooks (including third-party overrides) through the existing
    // virtual vector overload. Ordinary SRS transforms take the allocation-free path.
    if (typeid(*this) != typeid(SpatialReference) ||
        typeid(*outputSRS) != typeid(SpatialReference))
    {
        std::vector<osg::Vec3d> vectorPoints;
        if (count > 0u)
            vectorPoints.assign(points, points + count);

        const bool success = transform(vectorPoints, outputSRS);
        if (vectorPoints.size() != count)
            return false;

        if (count > 0u)
            std::copy(vectorPoints.begin(), vectorPoints.end(), points);
        return success;
    }

    return transformImpl(points, count, outputSRS, nullptr);
}
...
bool
SpatialReference::transformImpl(osg::Vec3d*                  points,
                                unsigned                     count,
                                const SpatialReference*      outputSRS,
                                std::vector<osg::Vec3d>*     vectorPoints) const
{
    OE_SOFT_ASSERT_AND_RETURN(outputSRS!=nullptr, false);
    OE_SOFT_ASSERT_AND_RETURN(points != nullptr || count == 0u, false);
    ...
    const SpatialReference* inputSRS = this;
    if (vectorPoints)
    {
        inputSRS = preTransform(*vectorPoints);
        points = vectorPoints->empty() ? nullptr : vectorPoints->data();
        count = static_cast<unsigned>(vectorPoints->size());
    }
    ...
    if ( inputSRS->isGeocentric() && !outputSRS->isGeocentric() )
    {
        ...
        geocentricToGeodetic(points, count, outputGeoSRS->getEllipsoid());
        return vectorPoints ?
            outputGeoSRS->transform(*vectorPoints, outputSRS) :
            outputGeoSRS->transform(points, count, outputSRS);
    }

    else if ( !inputSRS->isGeocentric() && outputSRS->isGeocentric() )
    {
        const SpatialReference* outputGeoSRS = outputSRS->getGeodeticSRS();
        success = vectorPoints ?
            inputSRS->transform(*vectorPoints, outputGeoSRS) :
            inputSRS->transform(points, count, outputGeoSRS);
        ...
        geodeticToGeocentric(points, count, outputGeoSRS->getEllipsoid());
        return success;
    }
    ...
    {
        z_done = inputSRS->transformZ(points, count, outputSRS, true);
    }
    ...
    if (success)
    {
        ...
        if ( !z_done )
        {
            z_done = inputSRS->transformZ(points, count, outputSRS, outputSRS->isGeographic());
        }   

        // run the user post-transform code
        if (vectorPoints)
            outputSRS->postTransform(*vectorPoints);
    }
...
{  
    ...
    auto& info = xformCache().get(getWKT(), out_srs->getWKT());
...
bool
SpatialReference::transformZ(osg::Vec3d*              points,
                             unsigned                 count,
                             const SpatialReference*  outputSRS,
                             bool                     pointsAreLatLong) const
{
    ...
    else // need to xform input points
    {
        ...
        static thread_local std::vector<osg::Vec3d> geopoints;
        geopoints.resize(count);
        if (count > 0u)
            std::copy(points, points + count, geopoints.begin());
        transform(geopoints.data(), count, getGeographicSRS());
    ...
    double& in_out_ymax) const
{
    ...
    constexpr unsigned edgeSamples = 9u;     // includes endpoints
    constexpr unsigned interiorSamples = 5u; // 5x5 interior grid (excluding edges)
    constexpr unsigned sampleCount =
        1u + edgeSamples * 4u + (interiorSamples - 2u) * (interiorSamples - 2u);

    std::array<osg::Vec3d, sampleCount> samples;
    unsigned numSamples = 0u;

    // Centroid first (used as a stable unwrap reference after transform).
    samples[numSamples++].set(
        in_out_xmin + width * 0.5,
        in_out_ymin + height * 0.5,
        0.0);
    ...
    for (unsigned int i = 0; i < edgeSamples; ++i)
    {
        ...
        samples[numSamples++].set(in_out_xmin, y, 0.0); // left
        samples[numSamples++].set(in_out_xmax, y, 0.0); // right
        samples[numSamples++].set(x, in_out_ymax, 0.0); // top
        samples[numSamples++].set(x, in_out_ymin, 0.0); // bottom
    }
    ...
    for (unsigned int iy = 1; iy + 1 < interiorSamples; ++iy)
    {
        ...
        for (unsigned int ix = 1; ix + 1 < interiorSamples; ++ix)
        {
            ...
            samples[numSamples++].set(x, y, 0.0);
        }
    }

    // Fast path: transform all at once.
    std::array<osg::Vec3d, sampleCount> transformed = samples;
    unsigned numTransformed = numSamples;
    bool ok = transform(transformed.data(), numTransformed, to_srs);
    ...
    if (!ok)
    {
        ...
        for (unsigned i = 0; i < numSamples; ++i)
        {
            ...
            if (transform(samples[i], to_srs, out))
                transformed[numTransformed++] = out;
        }
    }

    if (numTransformed == 0u)
        return false;
    ...
    if (to_srs->isGeographic())
    {
        ...
        for (unsigned i = 0; i < numTransformed; ++i)
        {
    ...
    unsigned int numx, unsigned int numy ) const
{
    ...
    points.resize(static_cast<std::size_t>(numx) * static_cast<std::size_t>(numy));
    ...
    for (unsigned int c = 0; c < numx; ++c, ++fc)
    {
        ...
        for (unsigned int r = 0; r < numy; ++r, ++fr)
        {
            ...
            points[pixel].set(dest_x, dest_y, 0.0);
            pixel++;     
        }
    }
